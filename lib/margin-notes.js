jQuery(document).ready(function($){
	...
	// if annotation text should be placed in the margins
	...
	//else, i.e. if annotations should be displayed as tooltips
		...
		$( '.mn-highlight' ).each(function(){
		    ...
		    for (let i=0; i < text.length; i++ ){
		      wrap += '<span class="wrap">'+text.slice(i,i+1)+'</span>';
		    }
		    $(this).html(wrap);
		    parentY = $('.wrap',$(this)).eq(0).offset().top;
		    let string = '<span class="mn-line" data-left="0">'
		    $('.wrap').each(function(){
		      let char = $(this).text();
		      offset = $(this).offset().top;
		      //console.log( offset > parentY );
		      if ( offset > parentY ){
